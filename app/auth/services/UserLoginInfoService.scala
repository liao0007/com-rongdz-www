...
import models.user.{LoginInfo => UserLoginInfo}

import scala.concurrent.Future

class UserLoginInfoService {

  def retrieve(loginInfo: SilhouetteLoginInfo): Future[Option[UserLoginInfo]] = Future.successful {
    UserLoginInfo.findBy("providerId" -> loginInfo.providerID, "providerKey" -> loginInfo.providerKey)
  }

  def save(silhouetteLoginInfo: SilhouetteLoginInfo, userId: Long): Future[UserLoginInfo] = Future.successful {
